...
# Usage: ./kernel/tests/boot-test.test.sh [-v] [--arch=riscv|arm64|amd64] [--boot=kernel|image|iso]
...
#   ./kernel/tests/boot-test.test.sh --arch=amd64        # Run AMD64 tests (kernel and image)
#   ./kernel/tests/boot-test.test.sh --arch=amd64 --boot=kernel # Run AMD64 kernel test only
#   ./kernel/tests/boot-test.test.sh -v --arch=arm64     # Run ARM64 tests with verbose output
#   ./kernel/tests/boot-test.test.sh --arch=arm64 --boot=image  # Run ARM64 disk image test
#   ./kernel/tests/boot-test.test.sh --arch=riscv,amd64 --boot=kernel  # Run RISC-V and AMD64 kernel tests
...
for arch in $TEST_MATRIX_ARCH; do
    for boot_type in $TEST_MATRIX_BOOT_TYPE; do
        qemu_cmd=$(get_full_qemu_cmd "$arch" "$boot_type" "log=debug")
